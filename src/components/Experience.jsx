...
export default function Experience() {
  return (
    <SectionWrapper id="experiencia" className="relative overflow-hidden">
      ...
      {/* Background mesh — warm indigo tint */}
      ...
      <div className="relative z-10">
        ...
        <div className="relative">
          {/* Timeline line — gradient indigo → emerald */}
          <div className="absolute left-4 md:left-8 top-0 bottom-0 w-px bg-gradient-to-b from-secondary/50 via-secondary/20 to-accent/30" />

          <div className="space-y-10">
            {experience.map((item, i) => {
              const isEmerald = i % 2 !== 0;
              const dotColor = isEmerald ? "bg-accent" : "bg-secondary";
              const periodColor = isEmerald ? "text-accent" : "text-secondary";
              ...
              return (
                <AnimatedElement key={i} delay={i * 0.1}>
                  <div className="relative pl-12 md:pl-20">
                    {/* Timeline dot */}
                    <div className={`absolute left-2.5 md:left-6.5 top-1.5 w-3 h-3 rounded-full ${dotColor} border-2 border-[#0a0f1e]`} />

                    {/* Period */}
                    <span className={`text-xs font-mono ${periodColor} font-semibold`}>
                      {item.period}
                    </span>

                    {/* Card content */}
                    <div className={`mt-2 bg-white/[0.02] rounded-xl p-5 border border-white/[0.06] transition-all duration-300 ${glowStyle}`}>
